...
from memory import build_memory_context, extract_memories, load_memories, save_memory
...
def save_state(run_id: str, state: AgentState) -> None:
    ...
    print_color(f"[MEMORY][SAVE] State saved | run_id={run_id}", Color.PINK)


def load_or_create_state(run_id: str) -> AgentState:
    print_color(f"[MEMORY][LOAD] Loading state | run_id={run_id}", Color.PINK)
    path = Path("states") / f"{run_id}.json"

    if not path.exists():
        print_color(f"[MEMORY][LOAD] State file not found | path={path}", Color.PINK)
...
def sync_state_with_history(
    ...
    calls_by_id = {}
    print_color("[MEMORY][SYNC] Syncing state with history", Color.YELLOW)
    for item in history:
    ...
    print_color("[MEMORY][SYNC] Sync completed", Color.YELLOW)

    return state


def remember_user_input(user_id: str, user_input: str) -> None:

    try:
        extracted_memories = extract_memories(
            client=client, model=openai_model, user_input=user_input
        )

        for content in extracted_memories:
            save_memory(user_id=user_id, content=content)

    except Exception as error:
        print_color(
            f"[MEMORY][EXTRACT][ERROR] Memory extraction failed | {error!r}", Color.RED
        )
...
def run_agent(
    ...
    print_color(f"[RUN][START] Running task | run_id={run_id}", color=Color.PINK)
    print()
    ...
    if state.done:
        print_color(
            f"[RUN][SKIP] Task already completed | run_id={run_id}", Color.PURPLE
        )
        return AgentRun(
            state=state,
        ...
        print_color(
            f"[RUN][CONTINUE] Task needs more work | run_id={run_id}",
            Color.PURPLE,
        )

    memory_context = ""
    if user_id is not None:
        memory_context = build_memory_context(load_memories(user_id))
...
"""
            ...
            if not tool_was_called:
                ...
                save_state(run_id, state)
                print_color(
                    f"[AGENT][FINAL] {agent_name} → {llm_resp.output_text}",
                    Color.GREEN,
                )
        ...
        save_state(run_id, state)
        print_color(
            f"[AGENT][ERROR] {agent_name} failed | {e!r}",
            Color.RED,
        )
...
def main() -> None:
    result = run_agent(
        ...
        system_prompt=ORCHESTRATOR_SYSTEM_PROMPT,
        task=(
            "What do you know about my programming background "
    ...
    print_color("[RESULT][AFTER_RESTART] Second run result", Color.GREEN)
