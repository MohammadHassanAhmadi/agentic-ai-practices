...
{result.result}
        ...
        response = client.responses.parse(
            model=openai_model, input=prompt, text_format=LLMJudgeResult
        )
...
def load_cases(cases_dir: Path = CASES_DIR) -> list[TestCase]:
    ...
    for file in sorted(cases_dir.glob("*.json")):
        try:
            ...
            print_color(
                f"[EVALUATOR][INVALID] {file.name} | {type(error).__name__}: {error}",
                Color.RED,
            )
...
def main() -> int:
    ...
    if args.runs <= 0:
        print_color("[EVALUATOR][ERROR] --runs must be greater than zero", Color.YELLOW)
        return 1
    ...
    if len(all_cases) == 0:
        ...
        print_color(f"[EVALUATOR][WARNING] {message}", Color.YELLOW)
        return 1
    ...
    if not valid_cases:
        print_color("[EVALUATOR][ERROR] No valid cases to run", Color.YELLOW)
        return 1
